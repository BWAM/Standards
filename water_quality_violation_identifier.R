...
# Input data tables
classification_site_info<-paste(inputdata_Location,"SITE FILE 2017.csv", sep="")
#chemistry_table<-paste(inputdata_Location,"Wallkill_water_chemistry_2017.csv",sep="")#<--als data
...
chemistry_table<-paste(outputdata_Location,"Test_RIBS_Wallkill_chemistry_table_2017.csv",sep="")
...
if (analysis_lab=="ALS"){
  ...
  data <- subset(chem_data, chem_data$sys_loc_code!="Method Blank" & chem_data$sys_loc_code!="LAB CONTROL SAMPLE" & chem_data$sys_loc_code!="BLANK" & chem_data$sys_loc_code!="Lab Control Sample" & chem_data$sys_loc_code!= "Duplicate Lab Control Sample")
  
  # Subset data to remove results with a detection flag == "N" and records w/out result values
  data <-subset(data, data$detect_flag == "Y" & !is.na(data$result_value))
...
if (analysis_lab=="USGS"){
  ...
  data_detect_issues <- subset(data, data$REMRK==">" | data$REMRK=="<")
...
if (analysis_lab=="ALS"){  
     ...
     ##################################################
     ### REALLY LONG METHOD SERVING AS ALTERNATIVE  ###
     ### TO IFELSE() STATEMENT                      ###
...
for(classes in data_subset_classes){
  ...
  for(i in 1:nsam){
    ...
    if(water_hardness %in% sample[[parameter_column]]){
      ...
      paramsAC<-c(water_cadmium,water_copper,water_fluoride,water_fluoride,water_lead,water_nickel,water_zinc)
    ...
    if(analysis_lab=="ALS"){
      ...
      keep <- c("cas_rn","Health..Water.Source.", "Health.Fish.Consumption.", "Aquatic.Chronic.", "Aquatic..Acute.", "Wildlife", "Aesthetic.Water.Source.", "Aesthetic.Food.Source.", "Recreation")
      standsample <- standsample[keep]
      rm(keep)
      
      # Merge sample df with standards table keeping only matching records
      sampleoutput <- merge(sample,standsample,by=c("cas_rn"), all=FALSE)
      ...
      sampleoutput <- merge(sampleoutput,sample,by=c("BASIN","LOCATION","RIVMILE","SAMPLE_DATE","SAMPLE_TIME","CLASSIFICA","STANDARD","PWL_ID","UNITS","VALUE","sample","p_sample"),all=TRUE)#,"Parameter.Names"
    ...
    if(analysis_lab=="USGS"){
      ...
      # Restrict standards table to desired fields based on analysis lab
      keep <- c("PCODE","Health..Water.Source.", "Health.Fish.Consumption.", "Aquatic.Chronic.", "Aquatic..Acute.", "Wildlife", "Aesthetic.Water.Source.", "Aesthetic.Food.Source.", "Recreation")
      standsample <- standsample[keep]#, "Parameter.Names"
      rm(keep)
      
      # Merge sample df with standards table keeping only matching records
      sampleoutput <- merge(sample,standsample,by=c("PCODE"))
      ...
      # Merge compared parameter records back with sample group and keep all additional parameters
      sampleoutput <- merge(sampleoutput,sample,by=c("LOCATION","RIVMILE","STAID","LOCAL","DATES","TIMES","MEDIM","STYPE","SAMPL","LABNO","PRJCT","ASTAT","LATDD","LNGDD","PCODE","Parameter.Names","DQIND",
                                                     "ANENT","CLASSIFICA","STANDARD","PWL_ID", "UNITS","REMRK","VALUE","sample"),all=TRUE)
    ...
    if(water_ph %in% sample[[parameter_column]]){
      ...
      if (classes=="classA"| classes=="classB" | classes=="classC"){
        ...
        sampleoutput$Aquatic.Chronic.[(sampleoutput[[parameter_column]]==water_ph & sampleoutput$p_sample==sample$p_sample)]<-pH_AC_standard
      }
      
      if (classes=="classA"){
        pH_HWS_standard<-paste(pH_HWS_high_standard,pH_HWS_low_standard,sep="-")
        sampleoutput$Health..Water.Source.[(sampleoutput[[parameter_column]]==water_ph & sampleoutput$p_sample==sample$p_sample)]<-pH_HWS_standard
      }
      if (classes=="classD"){
        pH_AA_standard<-paste(pH_AA_high_standard,pH_AA_low_standard,sep="-")
        sampleoutput$Aquatic..Acute.[(sampleoutput[[parameter_column]]==water_ph & sampleoutput$p_sample==sample$p_sample)]<-pH_AA_standard
      }
      
      # Compare pH to standards and write determination into output table
      #if (nsam>0){
        sampleoutput$AC[(sampleoutput[[parameter_column]]==water_ph & sampleoutput$p_sample==sample$p_sample)]<-ifelse(is.na(sampleoutput$Aquatic.Chronic.[(sampleoutput[[parameter_column]]==water_ph & sampleoutput$p_sample==sample$p_sample)]),"NS",
                                                                      ifelse(pH_value>pH_AC_high_standard,"V",ifelse(pH_value<pH_AC_low_standard,"V", "NV")))
        
        sampleoutput$HWS[(sampleoutput[[parameter_column]]==water_ph & sampleoutput$p_sample==sample$p_sample)]<-ifelse(is.na(sampleoutput$Health..Water.Source.[(sampleoutput[[parameter_column]]==water_ph & sampleoutput$p_sample==sample$p_sample)]),"NS",
                                                                       ifelse(pH_value>pH_HWS_high_standard,"V",ifelse(pH_value<pH_HWS_low_standard, "V", "NV")))
        
        sampleoutput$AA[(sampleoutput[[parameter_column]]==water_ph & sampleoutput$p_sample==sample$p_sample)]<-ifelse(is.na(sampleoutput$Aquatic..Acute.[(sampleoutput[[parameter_column]]==water_ph & sampleoutput$p_sample==sample$p_sample)]),"NS",
                                                                                                                  ifelse(pH_value>pH_HWS_high_standard,"V",ifelse(pH_value<pH_HWS_low_standard, "V", "NV")))
      #}#<- Closes function to compare pH values to standards
      
      #Fill in the fields that do not have a pH standard
      sampleoutput$HFS[(sampleoutput[[parameter_column]]==water_ph & sampleoutput$p_sample==sample$p_sample)]<-"NS"
      sampleoutput$AA[(sampleoutput[[parameter_column]]==water_ph & sampleoutput$p_sample==sample$p_sample)]<-"NS"
      sampleoutput$W[(sampleoutput[[parameter_column]]==water_ph & sampleoutput$p_sample==sample$p_sample)]<-"NS"
      sampleoutput$EWS[(sampleoutput[[parameter_column]]==water_ph & sampleoutput$p_sample==sample$p_sample)]<-"NS"
      sampleoutput$EFS[(sampleoutput[[parameter_column]]==water_ph & sampleoutput$p_sample==sample$p_sample)]<-"NS"
      sampleoutput$R[(sampleoutput[[parameter_column]]==water_ph & sampleoutput$p_sample==sample$p_sample)]<-"NS"
    ...
    if(water_dissolved_oxygen %in% sample[[parameter_column]]){ 
      ...
      if(sample$STANDARD[sample[[parameter_column]]==dissolved_oxygen]=="A"){
        sampleoutput$AC[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-ifelse(sample$VALUE[sample[[parameter_column]]==water_dissolved_oxygen]<diss_oxygen$A, "V", "NV")
        sampleoutput$Aquatic.Chronic.[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-diss_oxygen$A
      }
      if(sample$STANDARD[sample[[parameter_column]]==dissolved_oxygen]=="AT" | sample$STANDARD[sample[[parameter_column]]==dissolved_oxygen]=="A(T)"){
        sampleoutput$AC[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-ifelse(sample$VALUE[sample[[parameter_column]]==water_dissolved_oxygen]<diss_oxygen$A.T., "V", "NV")
        sampleoutput$Aquatic.Chronic.[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-diss_oxygen$A.T.
      }
      if(sample$STANDARD[sample[[parameter_column]]==dissolved_oxygen]=="ATS" | sample$STANDARD[sample[[parameter_column]]==dissolved_oxygen]=="A(TS)"){
        sampleoutput$AC[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-ifelse(sample$VALUE[sample[[parameter_column]]==water_dissolved_oxygen]<diss_oxygen$A.TS.,"V", "NV")
        sampleoutput$Aquatic.Chronic.[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-diss_oxygen$A.TS.
      }
      ### class AA
      if(sample$STANDARD[sample[[parameter_column]]==dissolved_oxygen]=="AA"){
        sampleoutput$AC[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-ifelse(sample$VALUE[sample[[parameter_column]]==water_dissolved_oxygen]<diss_oxygen$AA, "V", "NV")
        sampleoutput$Aquatic.Chronic.[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-diss_oxygen$AA
      }
      if(sample$STANDARD[sample[[parameter_column]]==dissolved_oxygen]=="AAT" | sample$STANDARD[sample[[parameter_column]]==dissolved_oxygen]=="AA(T)"){
        sampleoutput$AC[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-ifelse(sample$VALUE[sample[[parameter_column]]==water_dissolved_oxygen]<diss_oxygen$AA.T, "V", "NV")
        sampleoutput$Aquatic.Chronic.[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-diss_oxygen$AA.T
      }
      if(sample$STANDARD[sample[[parameter_column]]==dissolved_oxygen]=="AATS" | sample$STANDARD[sample[[parameter_column]]==dissolved_oxygen]=="AA(TS)"){
        sampleoutput$AC[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-ifelse(sample$VALUE[sample[[parameter_column]]==water_dissolved_oxygen]<diss_oxygen$AA.TS., "V", "NV")
        sampleoutput$Aquatic.Chronic.[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-diss_oxygen$AA.TS.
      }
      ### class A-S
      if(sample$STANDARD[sample[[parameter_column]]==dissolved_oxygen]=="AS" | sample$STANDARD[sample[[parameter_column]]==dissolved_oxygen]=="A-S"){
        sampleoutput$AC[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-ifelse(sample$VALUE[sample[[parameter_column]]==water_dissolved_oxygen]<diss_oxygen$A.S, "V", "NV")
        sampleoutput$Aquatic.Chronic.[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-diss_oxygen$A.S
      }
      if(sample$STANDARD[sample[[parameter_column]]==dissolved_oxygen]=="AST" | sample$STANDARD[sample[[parameter_column]]==dissolved_oxygen]=="AS(T)" |
         sample$STANDARD[sample[[parameter_column]]==dissolved_oxygen]=="A-ST" | sample$STANDARD[sample[[parameter_column]]==dissolved_oxygen]=="A-S(T)"){
        sampleoutput$AC[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-ifelse(sample$VALUE[sample[[parameter_column]]==water_dissolved_oxygen]<diss_oxygen$A.S.T., "V", "NV")
        sampleoutput$Aquatic.Chronic.[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-diss_oxygen$A.S.T.
      }
      if(sample$STANDARD[sample[[parameter_column]]==dissolved_oxygen]=="ASTS" | sample$STANDARD[sample[[parameter_column]]==dissolved_oxygen]=="A-STS" |
         sample$STANDARD[sample[[parameter_column]]==dissolved_oxygen]=="AS(TS)" |  sample$STANDARD[sample[[parameter_column]]==dissolved_oxygen]=="A-S(TS)"){
        sampleoutput$AC[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-ifelse(sample$VALUE[sample[[parameter_column]]==water_dissolved_oxygen]<diss_oxygen$A.S.TS., "V", "NV")
        sampleoutput$Aquatic.Chronic.[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-diss_oxygen$A.S.TS.
      }
      ### class AA-S
      if(sample$STANDARD[sample[[parameter_column]]==dissolved_oxygen]=="AAS" | sample$STANDARD[sample[[parameter_column]]==dissolved_oxygen]=="AA-S"){
        sampleoutput$AC[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-ifelse(sample$VALUE[sample[[parameter_column]]==water_dissolved_oxygen]<diss_oxygen$AA.S, "V", "NV")
        sampleoutput$Aquatic.Chronic.[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-diss_oxygen$AA.S
      }
      if(sample$STANDARD[sample[[parameter_column]]==dissolved_oxygen]=="AAST" | sample$STANDARD[sample[[parameter_column]]==dissolved_oxygen]=="AA-S(T)" | 
         sample$STANDARD[sample[[parameter_column]]==dissolved_oxygen]=="AAS(T)" | sample$STANDARD[sample[[parameter_column]]==dissolved_oxygen]=="AA-ST"){
        sampleoutput$AC[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-ifelse(sample$VALUE[sample[[parameter_column]]==water_dissolved_oxygen]<diss_oxygen$AA.S.T., "V", "NV")
        sampleoutput$Aquatic.Chronic.[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-diss_oxygen$AA.S.T.
      }
      if(sample$STANDARD[sample[[parameter_column]]==dissolved_oxygen]=="AASTS" | sample$STANDARD[sample[[parameter_column]]==dissolved_oxygen]=="AA-S(TS)" | 
         sample$STANDARD[sample[[parameter_column]]==dissolved_oxygen]=="AAS(TS)" | sample$STANDARD[sample[[parameter_column]]==dissolved_oxygen]=="AA-STS"){
        sampleoutput$AC[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-ifelse(sample$VALUE[sample[[parameter_column]]==water_dissolved_oxygen]<diss_oxygen$AA.S.TS., "V", "NV")
        sampleoutput$Aquatic.Chronic.[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-diss_oxygen$AA.S.TS.
      }
      # Check for class B waterbodies and compare to D.O. standard
      if(sample$STANDARD[sample[[parameter_column]]==dissolved_oxygen]=="B"){
        sampleoutput$AC[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-ifelse(sample$VALUE[sample[[parameter_column]]==water_dissolved_oxygen]<diss_oxygen$B,"V", "NV")
        sampleoutput$Aquatic.Chronic.[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-diss_oxygen$B
      }
      if(sample$STANDARD[sample[[parameter_column]]==dissolved_oxygen]=="B(T)" | sample$STANDARD[sample[[parameter_column]]==dissolved_oxygen]=="BT"){
        sampleoutput$AC[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-ifelse(sample$VALUE[sample[[parameter_column]]==water_dissolved_oxygen]<diss_oxygen$B.T.,"V", "NV")
        sampleoutput$Aquatic.Chronic.[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-diss_oxygen$B.T.
      }
      if(sample$STANDARD[sample[[parameter_column]]==dissolved_oxygen]=="B(TS)" | sample$STANDARD[sample[[parameter_column]]==dissolved_oxygen]=="BTS"){
        sampleoutput$AC[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-ifelse(sample$VALUE[sample[[parameter_column]]==water_dissolved_oxygen]<diss_oxygen$B.TS.,"V", "NV")
        sampleoutput$Aquatic.Chronic.[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-diss_oxygen$B.TS.
      }
      # Check for class C waterbodies and compare to D.O. standard
      if(sample$STANDARD[sample[[parameter_column]]==dissolved_oxygen]=="C"){
        sampleoutput$AC[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-ifelse(sample$VALUE[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<diss_oxygen$C,"V", "NV")
        sampleoutput$Aquatic.Chronic.[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-diss_oxygen$C
      }
      if(sample$STANDARD[sample[[parameter_column]]==dissolved_oxygen]=="C(T)" | sample$STANDARD[sample[[parameter_column]]==dissolved_oxygen]=="CT"){
        sampleoutput$AC[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-ifelse(sample$VALUE[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<diss_oxygen$C.T.,"V", "NV")
        sampleoutput$Aquatic.Chronic.[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-diss_oxygen$C.T.
      }
      if(sample$STANDARD[sample[[parameter_column]]==dissolved_oxygen]=="C(TS)" | sample$STANDARD[sample[[parameter_column]]==dissolved_oxygen]=="CTS"){
        sampleoutput$AC[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-ifelse(sample$VALUE[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<diss_oxygen$C.TS.,"V", "NV")
        sampleoutput$Aquatic.Chronic.[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-diss_oxygen$C.TS.
      }
      # Check for class D waterbodies and compare to D.O. standard
      if(sample$STANDARD[sample[[parameter_column]]==dissolved_oxygen]=="D"){
        sampleoutput$AC[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-ifelse(sample$VALUE[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<diss_oxygen$D,"V", "NV")
        sampleoutput$AA[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-ifelse(sample$VALUE[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<diss_oxygen$Acute,"V", "NV")
        sampleoutput$Aquatic.Chronic.[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-diss_oxygen$D
        sampleoutput$Aquatic..Acute.[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-diss_oxygen$D
      }
      # Compare value to the Aquatic Acute Standard
      #if (nsam>0){
      sampleoutput$AA[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-ifelse(sample$VALUE[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<diss_oxygen$Acute,"V", "NV")
      sampleoutput$Aquatic..Acute.[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-diss_oxygen$Acute
      #}#<- Closes function to compare dissolved oxygen values to the standard values
      
      # Fill in the fields that do not have a D.O. standard
      sampleoutput$HWS[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-"NS"
      sampleoutput$HFS[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-"NS"
      sampleoutput$W[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-"NS"
      sampleoutput$EWS[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-"NS"
      sampleoutput$EFS[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-"NS"
      sampleoutput$R[(sampleoutput[[parameter_column]]==dissolved_oxygen & sampleoutput$p_sample==sample$p_sample)]<-"NS"
    ...
    if(water_ammonia_as_N %in% sample[[parameter_column]]){
      ...
      if(water_temperature %in% sample[[parameter_column]]){
        ...
        #if(sampleoutput$STANDARD[(sampleoutput[[parameter_column]]==water_ammonia_as_N & sampleoutput$p_sample==sample$p_sample)]=="C"){print("yes")}
        ...
        if(sample$STANDARD[sample[[parameter_column]]==water_ammonia_as_N]=="A" | sample$STANDARD[sample[[parameter_column]]==water_ammonia_as_N]=="AA" | 
          ...
          r_x1 <- ifelse(pH_value>=6.5 & pH_value<6.75, r_x1<-6.5,ifelse(pH_value>=6.75 & pH_value<7, r_x1<-6.75,ifelse(pH_value>=7 & pH_value<7.25, r_x1<-7, ifelse(
            pH_value>=7.25 & pH_value<7.5, r_x1<-7.25,ifelse(pH_value>=7.5 & pH_value<7.75, r_x1<-7.5, ifelse(pH_value>=7.75 & pH_value<8, r_x1<-7.75, ifelse(
              pH_value>=8 & pH_value<=9, r_x1<-8, "pH out of bounds")))))))
          r_x2 <- ifelse(pH_value>=6.5 & pH_value<6.75, r_x2<-6.75,ifelse(pH_value>=6.75 & pH_value<7, r_x2<-7,ifelse(pH_value>=7 & pH_value<7.25, r_x2<-7.25, ifelse(
            pH_value>=7.25 & pH_value<7.5, r_x2<-7.5,ifelse(pH_value>=7.5 & pH_value<7.75, r_x2<-7.75, ifelse(pH_value>=7.75 & pH_value<8, r_x2<-8, ifelse(
              pH_value>=8 & pH_value<=9, r_x2<-9, "pH out of bounds")))))))
          ...
          sampleoutput$Aquatic.Chronic.[(sampleoutput[[parameter_column]]==water_ammonia_as_N & sampleoutput$p_sample==sample$p_sample)]<-interpol_standard
          
          # Compare UIA to interpolated standard value
          sampleoutput$AC[(sampleoutput[[parameter_column]]==water_ammonia_as_N & sampleoutput$p_sample==sample$p_sample)]<-ifelse(UIA>interpol_standard, "V", 
                                                             ifelse(UIA<interpol_standard, "NV", 
                                                                    ifelse(is.na(UIA), "UIA error, calculated as NA", 
                                                                           ifelse(is.na(interpol_standard), "interpolation error, standard is NA", 
                                                                                  ifelse(UIA==interpol_standard,"UIA equals standard", 
                                                                                         "error with UIA comparison, UIA neither >, <, or = interpol standard")))))
           ...
           sample$STANDARD[sample[[parameter_column]]==water_ammonia_as_N]=="C(TS)" | sample$STANDARD[sample[[parameter_column]]==water_ammonia_as_N]=="CTS")
        {
          ...
          # Interpolate for pH in standards table
          pH_interpol<-c_y1+(((pH_value-r_x1)/(r_x2-r_x1))*(c_y2-c_y1))
          temp_interpol<-c_y1+(((water_temperature-c_y3)/(c_y4-c_y3))*(c_y5-c_y1))
          interpol_standard<-((pH_interpol+temp_interpol)/2)
          sampleoutput$Aquatic.Chronic.[(sampleoutput[[parameter_column]]==water_ammonia_as_N & sampleoutput$p_sample==sample$p_sample)]<-interpol_standard
...
# Merge finalOutput df with original data set to keep all records including lab QC records

# Pull only columns and merge with excluded records for data continuity
if (analysis_lab == "ALS"){
  ...
  #finalOutput<-merge(finalOutput, chem_data, all=TRUE)#by=c("BASIN","LOCATION","RIVMILE","SAMPLE_DATE","SAMPLE_TIME","Parameter.Names","cas_rn","sample","p_sample")
  finalOutput<-join(finalOutput, chem_data, type="left")
